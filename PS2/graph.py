...
class WeightedEdge(Edge):
	...
	def __str__(self):
		return '{}->{}'.format(self.src, self.dest) + ' ({}, {})'.format(self.total_distance, self.outdoor_distance)
...
class Digraph(object):
	...
	def __str__(self):
		...
		edge_strs = []
		for edge, keys in zip(self.edges.values(), self.edges.keys()) :
			dest_list = list(islice(edge, 0, None, 3))
			t_d_list = list(islice(edge, 1, None, 3))
			o_d_list = list(islice(edge, 2, None, 3))
			for a, b, c in zip(dest_list, t_d_list, o_d_list):
				edge_strs.append('{}->{}'.format(keys, a) + ' ({}, {})'.format(b, c))
		return '\n'.join(edge_strs)
	...
	def add_edge(self, edge):
		...
		self.edges[src].append(total_distance)
		self.edges[src].append(outdoor_distance)
		
#need to be able to access total_distance and outdoor distance in relation to graph edges but cannot. Why?
#currently have a dictionary: {a: [b, c], b: [c], c: []} but it needs to be {{a: {b: x, y} , {c: x, y}} , {b: {c: x, y}, {c: {}}}
# ================================================================
